...
pub trait ModuleImpl {
	fn draw(&self, buf: &mut Buffer, bg: &Color, time: &DateTime<Local>) -> Result<Vec<(i32, i32, i32, i32)>, ::std::io::Error>;
...
impl Module {
	...
	pub fn draw(&self, buf: &mut Buffer, bg: &Color, time: &DateTime<Local>) -> Result<Vec<(i32, i32, i32, i32)>, ::std::io::Error> {
		self.m.lock().unwrap().draw(&mut buf.subdimensions(self.pos), bg, time)
	}
